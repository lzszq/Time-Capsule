...
from init_database import create_tables_of_TimeCapsule, put_data, get_keys, confirm_key, open_data, is_open_time, open_data_not_yet
...
def open_capsule():
    ...
    if request.method == 'POST':
        ...
        is_time = is_open_time(request.form['key'])
        ...
        elif flag and not is_time:
            content_type = 'open_not_yet'
            filled_data['key'] = request.form['key']
            filled_data = open_data_not_yet(request.form['key'], data_type, filled_data)
            html = render_template('open.html', content_type = content_type, filled_data = filled_data)
            return html
