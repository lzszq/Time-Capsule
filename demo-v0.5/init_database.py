...
def create_tables_of_TimeCapsule():
    ...
    cursor.execute('''create table if not exists `Capsule`(
        ...
        `content` text, 
        `tips` text,
        `is_send_email` tinyint default 0
        )charset=utf8;
    ''')
    conn.commit()
...
def select_is_send_email():
    ...
    cursor.execute('select `email` from `Capsule` where `is_send_email` = 0;')
    result = cursor.fetchall()
    user = []
    for data, in result:
        cursor.execute("select DATE_FORMAT(time, '%Y-%m-%d %H:%m:%S') from `Capsule` where `email` = %s;", (data,))
